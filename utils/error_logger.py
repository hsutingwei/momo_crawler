            ...
            with self.db.cursor() as cursor:
                query = '''
                    ...
                    (keyword, file_type, file_path, file_size, sync_status, 
                     sync_start_time, sync_end_time, error_message, created_at, updated_at)
                    VALUES (%s, %s, %s, %s, 'failed', NOW(), NOW(), %s, NOW(), NOW())
                    ON CONFLICT (keyword, file_type, file_path) DO UPDATE SET
                        sync_status = 'failed',
                        sync_end_time = NOW(),
            ...
            with self.db.cursor() as cursor:
                if status == 'processing':
                    query = '''
                        INSERT INTO file_sync_logs 
                        (keyword, file_type, file_path, file_size, sync_status, 
                         sync_start_time, created_at, updated_at)
                    ...
                    query = '''
                        ...
                        (keyword, file_type, file_path, file_size, total_records, valid_records, error_records,
                         sync_status, sync_start_time, sync_end_time, created_at, updated_at)
