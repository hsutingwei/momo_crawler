...
### 3.4 壓縮理論特徵 (Compression & Information Theory)
**目標**: 使用**資訊理論**和 **Kolmogorov 複雜度**的近似方法來偵測「範本化刷評」。

#### 理論基礎

> **核心概念**: 真實的人類語言具有高「資訊密度」（難以壓縮），而範本化的刷評具有高「重複性」（容易壓縮）。

**Kolmogorov 複雜度**: 一段文字的「最短描述長度」。
- 真實評論: "這個超好用會再買，包裝精美，客服態度佳" → 高複雜度（難壓縮）
- 範本刷評: "好好好好好好好好好好好好好好好好好好好" → 低複雜度（易壓縮）

---

#### 3.4.1 壓縮比例 (Compression Ratio)
**目標**: 測量文字的「可壓縮程度」，低壓縮比 = 高重複性 = 可疑。
...
# 計算壓縮比
feat_compression_ratio = len(original_bytes) / len(compressed_bytes)
```
...
| 壓縮比 | 意義 | 典型案例 |
|-------|------|---------|
| **> 3.0** | 高資訊密度（難壓縮） | 真實的多元討論："品質不錯，價格合理，配送快速，客服專業" |
| **2.0-3.0** | 中等資訊密度 | 正常評論："還可以，符合預期" |
| **1.5-2.0** | 低資訊密度 | 部分重複："好好好，不錯不錯" |
| **< 1.5** | 極低資訊密度（易壓縮） | 範本刷評："👍👍👍👍👍👍👍👍👍👍" |
...
**Zlib 壓縮原理**（LZ77 算法）:
1. 找到重複的字串
2. 用「指向前面出現位置的指標」取代重複內容
3. 重複越多，壓縮率越高

**範例**:
```
原文: "好好好好好好好好好好" (30 bytes)
壓縮: "好" + "重複9次" (約 10 bytes)
壓縮比: 30 / 10 = 3.0 (非常容易壓縮 = 可疑！)

原文: "品質很好，價格合理，配送快" (45 bytes)
壓縮: 約 38 bytes (幾乎無重複)
壓縮比: 45 / 38 = 1.18 (難壓縮 = 真實！)
```

**注意**: 壓縮比「越高」反而代表「重複性越低」→ 真實性越高

---

#### 3.4.2 標準化壓縮距離 (NCD - Normalized Compression Distance)
**目標**: 測量評論文字與「**已知垃圾範本**」的相似度。

##### 垃圾範本定義
**來源檔案**: [`data_loader.py:32-43`](file:///c:/yves/momo/momo_crawler/Model/data_loader.py#L32-L43)
...
其中：
- `x`: 待測評論文字
- `y`: 垃圾範本文字（所有 SPAM_REFERENCES 的合併）
- `C(x)`: 壓縮後的 x 長度
- `C(x + y)`: 壓縮後的「x 和 y 合併」長度
...
feat_ncd_spam = max(0.0, min(1.0, ncd))  # Clip to [0, 1]
```
...
| NCD 值 | 意義 | 解讀 |
|-------|------|------|
| **接近 0** | 評論與垃圾範本**高度相似** | 🚨 極可能是範本刷評 |
| **0.2-0.5** | 部分相似 | ⚠️ 可能使用部分範本句 |
| **0.5-0.8** | 低相似度 | ✅ 較為真實 |
| **接近 1** | 完全不相似 | ✅ 高度個性化，真實性高 |

##### 為什麼 NCD 能偵測相似度？

**直覺**:
- 如果「x + y」壓縮後的大小 ≈ max(C(x), C(y))，代表 x 和 y 有大量重複 → 相似！
- 如果「x + y」壓縮後的大小 ≈ C(x) + C(y)，代表 x 和 y 沒有重複 → 不相似！

**案例 1: 高相似度（範本刷評）**
```
x = "出貨速度快，包裝完整"  (C(x) = 30)
y = SPAM_REFERENCES          (C(y) = 200)
x + y 合併後有大量重複「出貨速度快，包裝完整」

C(x + y) = 210 (幾乎沒增加!)
NCD = (210 - 30) / 200 = 0.9 ... 等等，這不對

讓我重新計算：
NCD = (210 - min(30, 200)) / max(30, 200)
    = (210 - 30) / 200
    = 180 / 200 = 0.9

不對，NCD 低才代表相似...讓我再檢查公式
```

**更正**: 讓我重新理解 NCD 公式：

實際上 NCD 的正確理解是：
- **低 NCD** = 相似（因為合併後壓縮效率高，代表有共同資訊）
- **高 NCD** = 不相似（合併後壓縮效率低，代表沒有共同資訊）
...
| 特徵 | 偵測對象 | 原理 |
|-----|---------|------|
| **Compression Ratio** | 「自我重複」（如 "好好好好好"） | 內部重複性高 → 易壓縮 |
| **NCD Spam** | 「範本相似」（如 "出貨快速，包裝完整"） | 與已知範本相似 → 低 NCD |

**組合應用**:
- 高壓縮比 + 低 NCD → 🚨 確定是範本刷評
- 高壓縮比 + 高 NCD → ⚠️ 自我重複但不是已知範本（仍可疑）
- 低壓縮比 + 低 NCD → ⚠️ 使用範本但有加料（部分真實）
- 低壓縮比 + 高 NCD → ✅ 真實的個性化評論
...
**限制**:
1. **計算成本**: 需要逐條評論進行壓縮（較慢）
2. **範本更新**: SPAM_REFERENCES 需要定期更新（刷評範本會演化）
3. **語言依賴**: Zlib 對中文的壓縮效率可能不如英文

**改進方向**:
- 使用更先進的壓縮算法（如 LZMA）
- 動態更新垃圾範本庫
- 結合 BERT 語意相似度（更精準但更慢）

**結論**: 壓縮理論特徵提供了一個「無監督的範本檢測方法」，不需要人工標註「哪些是刷評」，只需要觀察「資訊密度」和「與已知範本的相似度」。
