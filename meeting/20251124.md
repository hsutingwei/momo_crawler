...
補充：為什麼初期先用 2025-06-25 當作 cutoff？

- **資料覆蓋的折衷點**：目前的爬蟲資料，cutoff 之前與之後都保有一定長度的觀察期。從前面 Temporal Feature 的分析可以看到，多數商品的最後評論距離 cutoff 大約落在 40 天左右，代表在 cutoff 前已有一段可用的歷史，而在 cutoff 後也還有足夠的時間讓「爆發事件」發生。
- **工程實作的起點**：現階段所有 SQL 與 data loader 的實作都基於單一 cutoff，若一開始就同時考慮多個 cutoff，實作與解釋複雜度會大幅提升。因此先在固定的 6/25 上，把 Label 設計與特徵工程走通、確認有效，之後再把同一套流程複製到其他日期做穩健性檢查。
...
另一個相關的疑慮是：在 2025-06-25 這一天，不同商品的「出現時間」與「被爬到的時間長度」並不相同，有些商品已經累積了很長的評論與銷售歷史，有些則是相對「新商品」。這會造成：

- 老商品的累積評論數、關鍵字出現次數自然偏高；
- 新商品在同一時間點可用的歷史訊號比較少。

本研究的處理方式是：

- 將這種差異視為「真實平台決策情境的一部分」：在真實情況下，平台在某一天做預測時，本來就會握有不同長度的歷史資訊。
- 在特徵設計上，盡量避免讓「年資」本身完全主導結果，因此除了累積指標，還加入了：
  - 近 90 天的評論數與比例（`comment_count_90d`, `comment_90d_ratio`）
  - 近 90 天的評分／情緒與關鍵字比例（`sentiment_mean_recent`, `repurchase_ratio_recent` 等）
- 這些 Temporal / Content Features 讓模型可以「比較老商品自己近期是否還活躍」、「新商品在短期內是否快速累積好評」，而不只是單純比累積總量。

更嚴格的做法（未來工作）會是：固定每個商品的觀察視窗長度（例如上線後前 60 天），再看之後的成長，但這需要額外重建「上線時間」或「第一個快照時間」，目前先留在 Future Work 中。

### 7.3 銷售快照（Sales Snapshots）時間不固定的影響
...
綜合而言，雖然 `sales_snapshots` 的時間分布並不完美，但所有實驗（baseline、hybrid、多類別、binary_explosive）皆在相同的快照資料與 cutoff 設定下進行，比較的重點是「在同一個不完美的現實資料條件下，不同 Label 設計與特徵組合是否能穩定帶來效果提升」，這樣的相對比較仍具有實務與研究價值。


---

## 8. 附錄：階段性實驗產出（Artifacts）
